...
public class AlbumSeedService {
    ...
    public AlbumSeedService(AlbumRepository albumRepository, UserRepository userRepository, ReviewRepository reviewRepository) {
    ...
    public void truncateAndSeedAlbums() {
        ...
        List<Album> samples = List.of(
                ...
                album("ASTROWORLD", "Travis Scott", "Theme-park rap fantasy with huge singles.", p("25.99"), "/images/astroworld.png", 2018,
                ...
                album("Get Rich or Die Tryin'", "50 Cent", "The definitive 2000s gangsta rap blockbuster.", p("21.99"), "/images/getrichordietryin.png", 2003,
                ...
                album("good kid, m.A.A.d city", "Kendrick Lamar", "Cinematic coming-of-age rap story.", p("23.49"), "/images/goodmidmaadcity.png", 2012,
        ...
        User expert = userRepository.findByEmail("expert@pitchfork.com").orElseGet(() -> {
            User u = new User();
            u.setEmail("expert@pitchfork.com");
            u.setFullName("Pitchfork Editor");
            u.setPassword("password"); // raw, ok for dummy
            return userRepository.save(u);
        });

        User fantano = userRepository.findByEmail("theneedledrop@youtube.com").orElseGet(() -> {
            User u = new User();
            u.setEmail("theneedledrop@youtube.com");
            u.setFullName("Anthony Fantano");
            u.setPassword("password");
            return userRepository.save(u);
        });

        for (Album album : albumRepository.findAll()) {
            if (album.getTitle().equals("DAMN.")) {
                reviewRepository.save(new Review(expert, album, 5, "A towering achievement that perfectly balances Kendrick's commercial instincts with his intellectual depth. Simply brilliant."));
                reviewRepository.save(new Review(fantano, album, 4, "Light to decent 7 on this one... kidding, it's a solid 9. The production is incredibly crisp."));
            } else if (album.getTitle().equals("Illmatic")) {
                reviewRepository.save(new Review(expert, album, 5, "The holy grail of East Coast hip-hop. Every single bar hits perfectly."));
            } else if (album.getTitle().equals("My Beautiful Dark Twisted Fantasy")) {
                reviewRepository.save(new Review(expert, album, 5, "A definitive masterpiece of our time. Maximalist, flawed, and utterly beautiful."));
            }
        }
    }
